...
func TestBinaryContains(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:  "integer in set",
			left:  NewSet(Integer(1), Integer(2), Integer(3)),
			right: Integer(1),
			want:  Bool(true),
		},
		{
			name:  "string not in set",
			left:  NewSet(syms.Insert("def"), syms.Insert("ijk")),
			right: syms.Insert("abc"),
			want:  Bool(false),
		},
		{
			name:  "bytes in set",
			left:  NewSet(Bytes("abc"), Bytes("def")),
			right: Bytes("abc"),
			want:  Bool(true),
		},
		{
			name:  "symbol not in set",
			left:  NewSet(String(1), String(2)),
			right: String(0),
			want:  Bool(false),
		},
		{
			name:  "set element type mismatch",
			left:  NewSet(Integer(1), Integer(2)),
			right: String(0),
			want:  Bool(false),
		},
		{
			name:  "bool in set",
			left:  NewSet(Bool(true), Bool(false)),
			right: Bool(true),
			want:  Bool(true),
		},
		{
			name:  "date not in set",
			left:  NewSet(Date(1), Date(2)),
			right: Date(0),
			want:  Bool(false),
		},
		{
			name:  "set not subset",
			left:  NewSet(Integer(1), Integer(2)),
			right: NewSet(Integer(0)),
			want:  Bool(false),
		},
		{
			name:  "set is subset",
			left:  NewSet(Integer(1), Integer(2)),
			right: NewSet(Integer(1)),
			want:  Bool(true),
		},
		{
